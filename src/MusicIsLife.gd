...
func _ready():
	...
	mat.emission_energy = 0 # start off black
	get_child(0).get_mesh().surface_set_material( 0, mat )
	set_colours(main.get_current_colour_preset()[0])

# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta):
	if (colour_index != main.get_current_colour_index()):
		colour_time_grad.set_color(0, main.get_colour_preset(colour_index)[0] )
		colour_time_grad.set_color(1, main.get_current_colour_preset()[0] )
	...
	mat.emission_energy = main.get_current_colour_preset()[2] * (stream.histogram.max() * 1.15)
